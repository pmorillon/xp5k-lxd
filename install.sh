...
# Create containers
for i in $(seq -f "lxc%02g" 1 20); do lxc launch ubuntu:xenial $i; done
...
for i in $(seq -f "lxc%02g" 1 20); do lxc file push /root/.ssh/authorized_keys $i/root/.ssh/authorized_keys; done
